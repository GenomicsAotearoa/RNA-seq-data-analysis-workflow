...
Exploratory analysis is a new section in this workflow, and will cover basic visualisations and characterisations of the data, then any initial transformations (e.g., library size) required before DGE. The decision to add this as a standalone section is based on both the Carpentries Incubator workshop and the HBC training workshop.

## What is exploratory analysis and why is it necessary?

Exploratory analysis is looking at the data. You are attempting to get a sense of what the data looks like, to notice outliers or issues (e.g., hey isn't it weird that one of my wt samples groups with all the mt samples, and one of the mut samples groups with the wt samples? I wonder what's happening there?).

If your data set was a physical problem (like a squeeky wheel or a well wrapped present) you'd physically explore it (pick it up, is it heavy, do I shake it?). Exploratory analysis is the data science equivalent.
...
That already looks much better. Now, let's split out the count data, removing the annotation (e.g., Chr, Start, End) columns and adding Geneid back into the object as the row names. Having the geneid as rownames is much easier than having that data as a column.
...
![Raw counts are highly skewed](images/rawCountsSkewed.png) This figure indicates we should consider logging our data (for the purpose of visualisations).
...
![Log counts are easier to visualise](images/logCounts.png)
...
![Library size are approximately similar across samples](images/librarySizeColour.png)
...
RNA-seq data has a property that can negatively impact the accuracy of certain statistical methods. This is a property called "heteroskedasticity": as the average expression level of a gene increases, so too does the standard deviation. In other words, the more highly expressed a gene is, the greater the variance we can expect to observe. Many statistical techniques assume that for most of the data the variance is independent on the mean.

Shortly we will use two different methods to identify differentially expressed genes, and each one will take a different approach to correct or adjust for heteroskedasticity. It is much more practical to look at these approaches while we identify differentially expressed genes, but it is worth noting early that heteroskedasticity is an issue that needs to be addressed. **This needs work**
